...
TOC (need to organize this way): 
1. Set Up (need to show code chunks, but unsure if we need to show packages)
2. Importing NJT Data
3. Importing Weather Data
4. Exploratory Analysis (Need to move up)
5. Feature Engineering 
6. Ride Panel (Need to move down? Or can be before 4?)
7. Models
8. Predictions 
9. Evaluation 
10. Conclusion
...
setwd("C:/Users/ammar/OneDrive/Desktop/CPLN592/Final")
...
test.reg.linear4 <- lm(log(delay_minutes) ~ hour(interval60) + dotw + (line * Peak) + Offender + lag1Hours + lag2Hours +lag3Hours + lag6Hours + lag24Hours + lag168Hours + Temperature + Wind_Speed + Rain + Trip_Count, data=ride.Train %>% mutate(delay_minutes = delay_minutes + 0000.1)) # STARGAZER HERE!!
...
Myron's notes as of 1:30am 
- THIS BITCH REALLY TRIED TO KILL ME BRO READ CAREFULLY LOL
...
test %>% # TRAIN ID
  ...
  kable(caption = "Train ID and average delay time by Line", caption = "Figure 5.4") %>%
  kable_styling("striped",full_width = F)
...
test_delays %>% 
  ...
  kable(caption = "Average delays during peak time by Line", caption = "Figure 5.5") %>%
  kable_styling("striped",full_width = F)
